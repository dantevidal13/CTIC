...
class DAOConsultas extends DBConnect{
	...
	function getUsuarios(){
		...
		$q = 'select id_usuario, username, nombres, apellido_paterno, apellido_materno, gr.nombre_grupo, un.codigo, created_at
		from tb_usuarios us left join tb_grupos gr on (us.id_grupo=gr.id_grupo) left join tb_unidad un on (us.id_unidad=un.codigo)';
		$stmt = $dbh->prepare($q);
		$stmt->execute();
...
left join tb_grupos gr on (us.id_grupo=gr.id_grupo) left join tb_unidad un on (us.id_unidad=un.codigo) left join tb_tipo_documento td on 
(us.id_tipo_documento=td.id_tipo_documento) where id_usuario=:id_usuario';
	...
	function getUnidad(){
		...
		$q = 'select codigo, nombre_unidad from tb_unidad';
		$stmt = $dbh->prepare($q);
		$stmt->execute();
		$r = $stmt->fetchAll(PDO::FETCH_ASSOC);
		return json_encode($r);
	...
	function getUnidad_by_id($dato){
		...
		$q = 'select *from tb_unidad where codigo= :codigo';
		$stmt = $dbh->prepare($q);
		$stmt->bindParam(':codigo',  $dato, PDO::PARAM_STR);
		$stmt->execute();
		$r = $stmt->fetchAll(PDO::FETCH_ASSOC);
		return json_encode($r);
		//return $r;
	...
	function eliminar_UNI($dato){
		...
		$q = 'delete from tb_unidad where codigo= :codigo';
		$stmt = $dbh->prepare($q);
		$stmt->bindParam(':codigo',  $dato, PDO::PARAM_STR);
		$stmt->execute();
		$r = $stmt->fetchAll(PDO::FETCH_ASSOC);
	...
	function crear_UNI($dato){
		...
		$q = 'INSERT INTO tb_unidad (`nombre_unidad`,`codigo`) values (:nombre_unidad, :codigo)';
		$stmt = $dbh->prepare($q);
		$stmt->bindParam(':nombre_unidad',  $dato->name, PDO::PARAM_STR);
		$stmt->bindParam(':codigo',  $dato->codigo, PDO::PARAM_STR);
		$stmt->execute();
		$r = $stmt->fetchAll(PDO::FETCH_ASSOC);
		return json_encode($r);
	}
	...
	function modificar_UNI($dato){
		...
		$q = 'UPDATE tb_unidad SET nombre_unidad=:nombre_unidad WHERE codigo=:codigo';
		$stmt = $dbh->prepare($q);
		$stmt->bindParam(':codigo',  $dato->codigo, PDO::PARAM_STR);
