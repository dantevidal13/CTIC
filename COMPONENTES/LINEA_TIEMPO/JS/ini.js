	...
	this.ini=function(){
		...
		$('.comp-linea-tiempo').each(function(){
			...
			if($(this).data('tipocarga')=='manual'){	
				...
				var html_linea_tiempo='<div id="comp-linea-tiempo-'+obj_aux.CONT+'"  data-indice="'+obj_aux.CONT+'">'+
					'<div class="fondos-linea-tiempo">'+
						html_fondos+				      
					'</div>'+
		...
		clearInterval(delay_banner);
		  ...
		  $(obj.ID_DOM+' .fondo').each(function(){
			...
			var delay_responsive=setInterval(function(){
				...
				$('.comp-linea-tiempo').removeClass('responsive');
				var width_texto=$('.div-info-hitos .seccion-hito .info span').css('width').replace('px','');
			
				if(width_texto<100){

					$('.comp-linea-tiempo').addClass('responsive');
